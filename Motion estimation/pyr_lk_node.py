# Main code running Pyramidal Lucas-Kanade optical flow on live camera feed
# Publishes delta and pose in pixels and mm, quality, and validity over ROS2 topics
# Camera records using YUV420 format for compatibility with rpicam-vid on ubuntu 24.04
import sys
import threading
import time
import subprocess
...
class CameraLKNode(Node):
    def __init__(self):
        super().__init__("camera_lk_node")
        ...
        self.reset_srv = self.create_service(Trigger, "/vision/lk_reset", self.on_reset)

        # LK parameters
        self.declare_parameter("mm_per_px", 0.157566)
        self.declare_parameter("scale", 0.15)
        self.declare_parameter("grid_spacing", 25)
        self.declare_parameter("win_size", 21)
        self.declare_parameter("max_level", 3)
        self.declare_parameter("criteria_count", 30)
        self.declare_parameter("criteria_eps", 0.01)
        self.declare_parameter("reinit_thresh", 0.3)
        self.declare_parameter("deadband_px", 0.3)
        ...
        self.mm_per_px = self.mm_per_px_base
        self.scale = float(self.get_parameter("scale").value)
        self.grid_spacing = int(self.get_parameter("grid_spacing").value)
        win_size = int(self.get_parameter("win_size").value)
        self.win_size = (win_size, win_size)
        self.max_level = int(self.get_parameter("max_level").value)
        crit_count = int(self.get_parameter("criteria_count").value)
        crit_eps = float(self.get_parameter("criteria_eps").value)
        self.criteria = (
            cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT,
            crit_count,
            crit_eps,
        )
        self.reinit_thresh = float(self.get_parameter("reinit_thresh").value)
        ...
        # LK publishers
        self.delta_px_pub = self.create_publisher(Vector3Stamped, "/vision/lk_delta_px", 10)
        self.pose_px_pub  = self.create_publisher(Vector3Stamped, "/vision/lk_pose_px", 10)
        self.delta_mm_pub = self.create_publisher(Vector3Stamped, "/vision/lk_delta_mm", 10)
        self.pose_mm_pub  = self.create_publisher(Vector3Stamped, "/vision/lk_pose_mm", 10)
        self.vel_mm_pub   = self.create_publisher(Vector3Stamped, "/vision/lk_vel_mm", 10)
        self.quality_pub  = self.create_publisher(Float32, "/vision/lk_quality", 10)
        self.valid_pub    = self.create_publisher(Int32, "/vision/lk_valid", 10)
        ...
        self.last_teensy_pi_rx = None      # Pi time when we received last timestamp
        self._uart_rx_buf = b""
        ...
        #     f"CameraLKNode started: {self.width}x{self.height} @ {self.fps} fps "
    ...
    # Defining the service for reseting pose. Needed for GUI button.
    def on_reset(self, request, response):
        self.reset_requested = True
        response.success = True
        response.message = "Optical flow reset requested."
        return response
                ...
                while b"\n" in self._uart_rx_buf:
                    ...
                    if line_str.startswith("T,"):
                        try:
                            ...
                            t_teensy = float(t_str)
                            self.last_teensy_time = t_teensy
    ...
    def _keyboard_loop(self):
        self.get_logger().info("Keyboard thread started. Press 'R' to reset LK.")
        try:
            while rclpy.ok():
                ch = sys.stdin.read(1)
                if not ch:
                    break
                if ch.lower() == "r":
                    self.get_logger().info("LK reset requested via keyboard.")
                    self.reset_requested = True
        except Exception as e:
            self.get_logger().warn(f"Keyboard loop error: {e}")

    # LK helpers
    def _make_grid_points(self, width, height, spacing):
        xs = np.arange(spacing // 2, width, spacing)
        ys = np.arange(spacing // 2, height, spacing)
    ...
    def _initialize_from_frame(self, gray_full):
        ...
        self.w_s = int(self.W_full * self.scale)
        self.h_s = int(self.H_full * self.scale)
        if self.w_s < 10 or self.h_s < 10:
            raise RuntimeError("Scaled image too small for LK.")
        ...
        self.p0 = self._make_grid_points(self.w_s, self.h_s, self.grid_spacing)
        self.n_points_initial = self.p0.shape[0]
        ...
        self.get_logger().info(
            f"LK initialized: full={self.W_full}x{self.H_full}, "
            f"scaled={self.w_s}x{self.h_s}, grid_points={self.n_points_initial}"
        )

    # Main frame processing
    def _process_frame(self, stamp, frame_bytes: bytes):
        """
        Process a single YUV420 frame: extract Y plane, run LK, publish outputs.
        `stamp` is a builtin_interfaces/Time message.
        """
        width = self.width
        height = self.height
        ...
        # Estimate Teensy time corresponding to the frame arrival
        ...
        if self.p0 is None or self.prev_gray_scaled is None or self.p0.shape[0] == 0:
            self.prev_gray_scaled = gray_scaled
            self.p0 = self._make_grid_points(self.w_s, self.h_s, self.grid_spacing)
            self.n_points_initial = self.p0.shape[0]
            self._publish_outputs(
        ...
        try:
            ...
            if self.consecutive_invalid_frames >= self.auto_reset_invalid_frames:
                self.get_logger().warn(
                    f"LK invalid for {self.consecutive_invalid_frames} frames. Auto-resetting."
                )
                self.reset_requested = True
        ...
        if p1 is not None and st is not None:
            ...
            if good_new.size > 0 and good_old.size > 0:
                ...
                inside_mask = (
                    (good_new[:, 0] >= 0) & (good_new[:, 0] < self.w_s) &
                    (good_new[:, 1] >= 0) & (good_new[:, 1] < self.h_s)
                )
                ...
                if good_new.shape[0] > 0:
                    ...
                    mad = np.median(np.abs(displacements - med), axis=0) + 1e-6
                    z = np.abs(displacements - med) / mad
                    inliers = np.all(z < 3.5, axis=1)

                    inlier_disp = displacements[inliers]
                    n_inliers = inlier_disp.shape[0]

                    if n_inliers > 0:
                        step_dx_scaled = float(np.median(inlier_disp[:, 0]))
                        step_dy_scaled = float(np.median(inlier_disp[:, 1]))
                    else:
                        step_dx_scaled = step_dy_scaled = 0.0

                    step_dx = step_dx_scaled / self.scale
                    step_dy = step_dy_scaled / self.scale

                    if abs(step_dx) < self.deadband_px:
                        step_dx = 0.0
                    if abs(step_dy) < self.deadband_px:
                        step_dy = 0.0
                    ...
                    if self.n_points_initial > 0:
                        survive_frac = self.p0.shape[0] / float(self.n_points_initial)
                        quality = n_inliers / float(self.n_points_initial)
                    else:
                        survive_frac = 0.0
                        quality = 0.0

                    valid = 1 if n_inliers > 0 else 0

                    if survive_frac < self.reinit_thresh:
                        self.get_logger().debug(
                            f"LK surviving tracks frac={survive_frac:.2f} < {self.reinit_thresh:.2f}, reinitializing grid."
                        )
                        self.p0 = self._make_grid_points(self.w_s, self.h_s, self.grid_spacing)
                        self.n_points_initial = self.p0.shape[0]
    ...
    def _tof_callback(self, msg: Float32):
        ...
        scale = distance_mm / ref
        new_mm_per_px = max(1e-6, self.mm_per_px_base * scale)
